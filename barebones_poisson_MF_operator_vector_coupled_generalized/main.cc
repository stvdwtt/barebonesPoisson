...
namespace matFreePoisson
{
                       ...
                       const std::pair<unsigned int,unsigned int>       &cell_range) const
  {
    ...
    private:
        ...
        static std::vector<const FiniteElement<dim>*> create_fe_list (const unsigned int polynomial_degree);
    ...
    fe_phi_scalar(QGaussLobatto<1>(2)),
    fe_phi(fe_phi_scalar,2),
    fe_c(QGaussLobatto<1>(2)),
    ...
    fe_vector(VectorElementDestroyer(create_fe_list (1)).get_data(),create_fe_multiplicities ()),
    ...
    // -----------------------------------------------------------------------------------------------------------------------------------------------------------
    // Methods to initialize the FESystem
    // -----------------------------------------------------------------------------------------------------------------------------------------------------------
    template <int dim>
    std::vector<const FiniteElement<dim>*> PoissonProblem<dim>::create_fe_list(const unsigned int polynomial_degree){
        std::vector<const FiniteElement<dim>*> temp;
        temp.push_back(new FE_Q<dim>(QGaussLobatto<1>(2)));
        temp.push_back(new FE_Q<dim>(QGaussLobatto<1>(2)));
        return temp;
    }

    template <int dim>
    std::vector<unsigned int> PoissonProblem<dim>::create_fe_multiplicities(){
    ...
    template <int dim>
    PoissonProblem<dim>::VectorElementDestroyer::
    VectorElementDestroyer (const std::vector<const FiniteElement<dim>*> &pointers)
    : data(pointers)
    {}
    template <int dim>
...
// -----------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    void PoissonProblem<dim>::setup_system ()
    {
        ...
        //
        system_matrix.clear();
        dof_handler.distribute_dofs (fe_vector);
