...
namespace matFreePoisson
{
  ...
  compute_diagonal()
  {
    ...
    for (unsigned int i=0; i<inverse_diagonal_vector.local_size(); ++i){
      std::cout << "diag_vector_element number: " << i << " value: " << inverse_diagonal_vector.local_element(i) << std::endl;
                       ...
                       const std::pair<unsigned int,unsigned int>       &cell_range) const
  {
    ...
    std::cout << "dofs_per_cell: " << phi.dofs_per_cell << " tensor_dofs_per_cell: " << phi.tensor_dofs_per_cell << std::endl;
    ...
    for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
      {
        ...
        //for (unsigned int i=0; i<phi.tensor_dofs_per_cell; ++i)
          ...
          for (unsigned int i=0; i<phi.tensor_dofs_per_cell*n_components; ++i){
              ...
              std::cout << "local_diagonal_vector number: " << i << " value: " << local_diagonal_vector[i][0] << " " << local_diagonal_vector[i][1] << std::endl;
          }
        phi.distribute_local_to_global (dst);
      }
  }
    ...
    fe (FE_Q<dim>(QGaussLobatto<1>(1+1)),number_of_components), // Question: Why doesn't this have to match the quadrature rule I put into the matrix free object? This is like Step 37 in this regard
    ...
    void PoissonProblem<dim>::setup_system ()
    {
        ...
        VectorTools::interpolate_boundary_values (dof_handler, 0, ZeroFunction<dim>(number_of_components), constraints);
        VectorTools::interpolate_boundary_values (dof_handler, 1, ZeroFunction<dim>(number_of_components), constraints);
        constraints.close();
        ...
        //Initial Condition
        VectorTools::interpolate (dof_handler,InitialCondition<dim> (number_of_components),Phi);
    ...
    local_apply_rhs (const MatrixFree<dim,double>               &data,
        ...
        const std::pair<unsigned int,unsigned int> &cell_range) const
        {
            FEEvaluation<dim,degree_finite_element,degree_finite_element+1, number_of_components> mat(data);
            //loop over all "cells"
            for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
            {
                mat.reinit (cell);
    ...
    void PoissonProblem<dim>::solve () {
        ...
        IterationNumberControl           solver_control (10, 1.0e-6,true);
    ...
    void PoissonProblem<dim>::output_results (const unsigned int cycle) const {
        ...
        if (number_of_components > 1)
            output_name.push_back("phi");
        data_out.add_data_vector (dof_handler, Phi, output_name,partofvector);
